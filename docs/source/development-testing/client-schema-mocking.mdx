...
We're ready to execute a query that includes our new field. Here's an abridged `GET_LAUNCH_DETAILS` query from the [Apollo full-stack tutorial](https://www.apollographql.com/tutorials/fullstack-quickstart/fetching-data-with-queries) with our `description` field added:
